...
*/
template<class I>
class TreeList : public TreeBin<I, int> {
 public:
...
/** cursore intero al nodo nella lista*/
    ...
    int nodes_level(int lv)const;
    int level()const;
    int higth()const;
    int pos_e(item elm)const;
 ...
 private:
	...
	struct Record {
		...
		/**riferimento al padre dei due figli*/
		//int parent;
        /** lista di posizioni intere di figli dello stesso array nodes, che dichiareremo dopo*/
		List_vector<int> childs;    //I figli possono essere massimo 2- albero binario
	};
...
/**calcola la posizione nell array di nodi del elemento considerato, se c'è*/
template <class I>
int TreeList<I>::pos_e(item elm)const
{
    int i=0;
    while(i<MAXNODES)
    {
...
template <class I>
int TreeList<I>::nodes_level(int lv)const
{
...
{   int ch=0;
    ...
    while(i<MAXNODES)
    {
        if(nodes[i].childs.empty() && nodes[i].used)
        {
            ch++;
        }
        i++;
...
si può incrementare il numero dei nodi, anche se n non è settato
*/
template <class I>
void TreeList<I>::insRoot (node n){
	if (empty()){
		_root = 0;
		//nodes[0].parent=ESITO_NEG;  //la radice non ha padre
		nodes[0].lv_e=1;    //la radice è sul primo livello
		nodes[0].used = true;
		numNodes++;
	}
}
...
typename TreeList<I>::node TreeList<I>::parent (node n) const{
    ...
    {  int lv_n= nodes[n].lv_e;
       int i=0;
       while(i<MAXNODES)
       {
           if(nodes[i].lv_e==lv_n-1)
           {
               int j=nodes[i].childs.begin();
               if(j==n)
               {
                   //nodes[n].parent=i;
                   return i;
               }
               j=nodes[i].childs.next(j);
               if(j==n)
               {
                   //nodes[n].parent=i;
                   return i;
...
bool TreeList<I>::leaf(node n) const{
	if(n>=0 && n<MAXNODES && nodes[n].used==true && hasParent(n))
...
typename TreeList<I>::node TreeList<I>::firstChild(node n) const{
    if(n>=0 && n<MAXNODES)
    {
        cout<<"primo figlio di "<<n<<" e' "<<nodes[n].childs.read(nodes[n].childs.begin())<<endl;
...
*/
template <class I>
int TreeList<I>::nextSibling(node n) const {
	if(n>=0 && n<MAXNODES)
    {
        if (!leaf(n)){
        int pos=nodes[n].childs.begin();
        cout<<"primo figlio di "<<parent(n)<<" e' "<<nodes[n].childs.begin();
        pos=nodes[n].childs.next(pos);
        cout<<" successivo figlio e' "<<pos<<endl;
		return(nodes[n].childs.read(pos));
	}
    }
}
...
void TreeList<I>::insFirst(node n, item el){
	...
	if(n>=0 && n<MAXNODES && nodes[n].used==true &&  nodes[n].childs.legginumElem()<nodes[n].childs.leggidim())   //posso inserire valori solo se non ho due figli
    {
       	int k;
...
void TreeList<I>::ins(node n, item el){
	// cerca una posizione libera
	if(n>=0 && n<MAXNODES && nodes[n].used==true &&  nodes[n].childs.legginumElem()<nodes[n].childs.leggidim())
    {
       	int k;
	for (k=0; k < MAXNODES && nodes[k].used == true; k++);
	if (k<MAXNODES){
